...
import { HandleChangeConfiguration } from '../HandleChangeConfiguration';
...
describe('HandleChangeConfiguration', () => {
  ...
  describe('execute', () => {
    it('should accept valid configuration change', async () => {
      const input: ChangeConfigurationInput = { chargePointId: 'CP-001', key: 'HeartbeatInterval', value: '60' };
    ...
    it('should require reboot for AuthorizationKey', async () => {
      const input: ChangeConfigurationInput = { chargePointId: 'CP-001', key: 'AuthorizationKey', value: 'newkey' };
      repository.find.mockResolvedValue({ id: 'CP-001' } as any);
      const result = await handler.execute(input);
    ...
    it('should require reboot for SecurityProfile', async () => {
      const input: ChangeConfigurationInput = { chargePointId: 'CP-001', key: 'SecurityProfile', value: '1' };
      repository.find.mockResolvedValue({ id: 'CP-001' } as any);
      const result = await handler.execute(input);
      expect(result.status).toBe('RebootRequired');
    });
    ...
    it('should reject if charge point not found', async () => {
      const input: ChangeConfigurationInput = { chargePointId: 'CP-NONEXISTENT', key: 'HeartbeatInterval', value: '60' };
